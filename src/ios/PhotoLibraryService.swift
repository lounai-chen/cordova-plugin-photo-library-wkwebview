...
final class PhotoLibraryService {
    ...
    class var instance: PhotoLibraryService {
     ...
     struct SingletonWrapper {
         nonisolated(unsafe) static let singleton = PhotoLibraryService()
     }
    ...
    @MainActor func requestAuthorization(_ success: @escaping () -> Void, failure: @escaping (_ err: String) -> Void ) {
    ...
    func saveImage(_ url: String, album: String, completion: @escaping (_ libraryItem: NSDictionary?, _ error: String?)->Void) {
        ...
        guard let data = try? Data(contentsOf: nsUrl) else {
            // 若无法加载数据（网络异常/URL 无效等），返回错误结果
            completion(nil, "无法从 URL 加载数据")
            return
        }
        ...
        PHPhotoLibrary.shared().performChanges({
            // 1. 创建图片保存请求，并保存请求对象（关键：通过请求对象获取插入的Asset ID）
            let creationRequest = PHAssetChangeRequest.creationRequestForAsset(from: img)
            // 2. 获取保存后的Asset的本地标识（localIdentifier），用于后续查询
            placeholder = creationRequest.placeholderForCreatedAsset
        }, completionHandler: { (success, error) in
            if success {
                //let localID =  placeholder?.localIdentifier
                // 只返回 localIdentifier，这是标准做法
               // let result = ["localIdentifier": localID]
                // completion(result, nil)
                
                let successDict: NSDictionary = [
                           "status": "success",          // 成功状态标识
                           "message": "图片保存成功"      // 成功提示字符串
                       ]
                completion(successDict, nil)
                
            } else {
                let errorMsg = error?.localizedDescription ?? "保存失败"
